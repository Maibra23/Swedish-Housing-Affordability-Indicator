...
**Test suite:** 681 passed, 0 failed, 1 skipped, 1 xfailed · 67/67 renders, also in a clean venv
...
`tests/test_no_hardcoded_counts.py` (92). Nineteen literals across nine files. Scope
...
`tests/test_runtime_dependencies.py` (10). The expected set is not written down: it is walked
...
are present and guarded by `tests/test_ui_affordances.py` (48); **visual equivalence is
...
`tests/test_css_naming.py` (7). `.lp-*` → `.shai-*`, `.riskklass-legend` → `.shai-risk-legend`
...
`tests/test_labels.py` (20). Two findings. **The register heading was off by one:**
...
`tests/test_docs_inventory.py` (135). 19 documents → **9 maintained** plus a 6-file archive.
...
`tests/test_design_system_doc.py` (25). The document was generated *from* the composed
...
| Check | Result |
|---|---|
| `pytest tests/` | **681 passed, 0 failed, 1 skipped, 1 xfailed** |
...
| `python scripts/audit.py` | **27 passed, 0 failed** — re-derived from the artifacts, not from the suite |
...
data; and 681 tests plus 67 renders run on a bare `pytest`.
...
|------|-------|---------|------------------------|
| 2026-09-23 | UI defects, APP_GUIDE | **Six interface defects fixed, and the page guide extended to the whole app.** The serious one was systemic: `delta_direction` chose both the KPI arrow and its colour, with the stylesheet hardcoding up as red and down as green — an assumption that only holds for risk-shaped numbers. Pages measuring the opposite bent the parameter to get the colour they wanted and broke the arrow, so the same quantity rendered as a green ▼ on Sida 01 (a 36,5 % **fall** in affordability shown as good), a red ▲ on Sida 03 and a ▼ beside a positive number on Sida 05. Split into direction and sentiment via `delta_meta`, guarded by `test_kpi_delta_orientation.py` (13). Also: log axis and explicit ticks for Versions A and C on Sida 02, which had squashed 2022–2024 flat against a linear axis, with Realversion (C) now the default tab; the Sida 03 forecast no longer stitches a county-level projection onto municipal history; histogram legend, median annotation and decimal separator; nav numbers removed. `ANALYSIS_GUIDE.md` renamed to **`APP_GUIDE.md`** and extended with sections for Sida 01, 02 and 03 in the same five-part shape, so all five analysis pages are documented rather than two; existing sections renumbered 1–8 to 4–11 and all 18 cross-references updated. | **R16 opened, High:** ARIMA's first forecast year is implausible for **21 of 21 counties**, every one collapsing to about a quarter of its last observed value in 2025 before rebounding, with the policy rate forecast to −0,88 % by 2029. Prophet does this for zero. The Sida 03 tab default was therefore left on Prophet and the page's own recommendation of ARIMA left standing, because resolving that contradiction the obvious way would show every reader a broken forecast on load. Writing section 2 also surfaced that Sida 02 and Sida 05 publish different numbers for the same county — mean of municipalities against county aggregate, diverging up to 2,65x with rank correlation 0,806 — now documented, and a product decision rather than a bug. |
| 2026-09-22 | R2, R3, R4, R5, R9, R10, R12, R15 | **The risk register has nothing OPEN.** Fifteen risks: thirteen closed, two accepted (R7 unpinned majors, R13 inlined CSS), zero open. Three closed by measuring rather than building: **R3** installs cleanly into an empty venv with wheels for prophet and pmdarima, **R4**'s OOM does not reproduce (197 MB peak, 121 s for all 84 series), and **R12**'s deferral reason did not apply because no panel holds a non-positive price, so the guard was value-neutral and the suite's last xfail is gone. **R5 and R10** were one problem: `build_panel.py` went 0 % to 91 % and then split into `clean_sources.py` (shapes), `build_panel.py` (joins) and `panel_summary.py` (the refresh report, previously 23 unreachable-by-test print statements); `riksbanken_client.py` 0 % to 87 %; the forecast pipelines tested by contract rather than by output, including a vintage check that would have caught the near-miss of 2026-09-21. `src/` overall 44 % to 73 %, nothing at 0 %. **R9** split ten markup blocks into `templates.py` under an enforced rule. **R15** was found and closed the same day. | Three guards found things their authors had not listed: the year-list scan found dead imputed-income shading on Sida 02 that could not render at all (T2.4's complete_case filter means the scored artifact has zero imputed rows); the prose guard found four stale figures inside `test_copy_matches_artifacts.py`'s own docstring; and the build_panel tests failed during the split itself, catching a patched module that left half the builder reading the real `data/raw/`. Residual accepted: the forecast model-fitting bodies are not unit-tested, deliberately — the contract on their output is the right instrument. |
