...
def selectable_years(path: Path | None = None) -> list[int]:
    """Every year a user may be offered, oldest first.

    The index runs from :func:`first_year` to :func:`complete_case_max_year`, and
    nothing outside that range can be displayed: earlier years have no policy
    ...
    read provenance since T1.4; this is the same fact, available to everything
    else.
    ...
    Returns:
        The inclusive year range the index covers.

    Raises:
        FileNotFoundError: If the artifact is missing.
        KeyError: If the artifact does not record the years.
    """
    return list(range(first_year(path), complete_case_max_year(path) + 1))
